...
import type {
  ...
  EventCreateInput,
  ProfileEvent,
} from "@/app/convo/profile/profile.types"
...
function EventsProfilePage({
  ...
  return (
    <section className="workspace-panel active">
      <div className="convo-profile-list">
        {events.length === 0 ? (
          <p className="contacts-state">No events saved yet.</p>
        ) : (
          events.map((profileEvent) => (
            <article className="convo-profile-row" key={profileEvent.id}>
              <div>
              ...
              <div className="convo-profile-row-actions">
                <button
      ...
      <form className="flat-form" onSubmit={handleSubmit}>
        ...
        <label className="convo-profile-check form-field-wide">
          <input
      ...
      {isEventLimitReached ? (
        <p className="convo-profile-message">You can save up to five events.</p>
      ) : null}
