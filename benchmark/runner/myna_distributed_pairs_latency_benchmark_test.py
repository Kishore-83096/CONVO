...
Run through the root benchmark entry point:
...
URL modes:
    ...
    # different host ports. Docker mode uses Compose service DNS names. The benchmark runner itself does not
    depend on generated container names.
...
DEFAULT_REPORT_ROOT = normalize_report_path_for_current_os(
    os.getenv("MYNA_REPORT_ROOT", "/reports")
)
DEFAULT_REPORT_DIR = str(Path(DEFAULT_REPORT_ROOT))
...
IDENTITY_DATABASE_URL = os.getenv("IDENTITY_DATABASE_URL", "").strip()
SERVICE_URL_PROFILES: dict[str, dict[str, str]] = {
    ...
    "docker": {
        "IDENTITY_BASE_URL": os.getenv("MYNA_DOCKER_IDENTITY_BASE_URL", "http://identity:5000"),
        "MESSENGER_BASE_URL": os.getenv("MYNA_DOCKER_MESSENGER_BASE_URL", "http://messenger:8000"),
    },
}
...
CONFIG: dict[str, Any] = {
    ...
    "CONNECTION_WARMUP_PARALLELISM": int(os.getenv("MYNA_CONNECTION_WARMUP_PARALLELISM", "20")),
    "CONNECTION_WARMUP_RETRY_ATTEMPTS": int(os.getenv("MYNA_CONNECTION_WARMUP_RETRY_ATTEMPTS", "4")),
    "CONNECTION_WARMUP_RETRY_DELAY_SECONDS": float(os.getenv("MYNA_CONNECTION_WARMUP_RETRY_DELAY_SECONDS", "0.25")),
    ...
    "OUTBOX_DRAIN_ENABLED": os.getenv("MYNA_OUTBOX_DRAIN_ENABLED", "true").strip().lower() in {"1", "true", "yes", "on"},
    "OUTBOX_DRAIN_TIMEOUT_SECONDS": float(os.getenv("MYNA_OUTBOX_DRAIN_TIMEOUT_SECONDS", "30")),
    "OUTBOX_DRAIN_POLL_SECONDS": float(os.getenv("MYNA_OUTBOX_DRAIN_POLL_SECONDS", "1")),
    "IDENTITY_BASE_URL_SOURCE": IDENTITY_BASE_URL_SOURCE,
    "MESSENGER_BASE_URL_SOURCE": MESSENGER_BASE_URL_SOURCE,
    "MESSENGER_DOCKER_CONTAINER": os.getenv("MYNA_MESSENGER_DOCKER_CONTAINER", "").strip(),
    "IDENTITY_DOCKER_CONTAINER": os.getenv("MYNA_IDENTITY_DOCKER_CONTAINER", "").strip(),
    "REDIS_DOCKER_CONTAINER": os.getenv("MYNA_REDIS_DOCKER_CONTAINER", "redis").strip(),
    "POSTGRES_DOCKER_CONTAINER": os.getenv("MYNA_POSTGRES_DOCKER_CONTAINER", "").strip(),
    "MESSENGER_PROJECT_ROOT": os.getenv("MYNA_MESSENGER_PROJECT_ROOT", "/app"),
...
def messenger_docker_cleanup_database_url() -> tuple[str | None, str]:
    ...
    return str(raw_url).strip(), source
...
def capture_postgres_connection_snapshot(stage: str) -> dict[str, Any]:
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    ...
                    SELECT
                        ...
                        )::bigint AS wait_event_present_connections,
                        COUNT(*) FILTER (
                            WHERE state = 'active'
                              AND wait_event IS NOT NULL
                        )::bigint AS active_wait_event_connections,
                        COUNT(*) FILTER (
                            WHERE wait_event_type = 'Lock'
                        )::bigint AS lock_waiting_connections,
                        COUNT(*) FILTER (
                            WHERE state = 'idle'
                              AND wait_event_type = 'Client'
                        )::bigint AS idle_client_wait_connections
                ...
                (
                    ...
                    active_connections,
                    idle_connections,
                    wait_event_present_connections,
                    active_wait_event_connections,
                    lock_waiting_connections,
                    idle_client_wait_connections,
                ) = cursor.fetchone()
                ...
                database_stats = cursor.fetchone() or (0, 0, 0, 0, 0)
                ...
                try:
                    cursor.execute(
                        ...
                        FROM pg_stat_activity
                        GROUP BY usename, client_addr, datname, state, backend_type
                    ...
                    activity_groups.append({"error": repr(exc)})

                return {
                    **base,
                    "available": True,
                    "max_connections": max_connections,
                    "connections_total": int(total_connections or 0),
                    "connections_active": int(active_connections or 0),
                    "connections_idle": int(idle_connections or 0),
                    "connections_wait_event_present": int(
                        wait_event_present_connections or 0
                    ),
                    "connections_active_wait_event": int(
                        active_wait_event_connections or 0
                    ),
                    "connections_lock_waiting": int(
                        lock_waiting_connections or 0
                    ),
                    "connections_idle_client_wait": int(
                        idle_client_wait_connections or 0
                    ),
                    # Backward-compatible field with corrected semantics.
                    "connections_waiting": int(
                        active_wait_event_connections or 0
                    ),
                    "database_numbackends": int(database_stats[0] or 0),
                    "xact_commit": int(database_stats[1] or 0),
                    "xact_rollback": int(database_stats[2] or 0),
                    "conflicts": int(database_stats[3] or 0),
                    "deadlocks": int(database_stats[4] or 0),
...
async def add_postgres_telemetry(report: dict[str, Any], stage: str) -> None:
    ...
    snapshot = await asyncio.to_thread(
        capture_postgres_connection_snapshot,
        stage,
    )
...
def configured_running_docker_containers() -> list[str]:
    ...
    names = [
        ...
        str(CONFIG["POSTGRES_DOCKER_CONTAINER"]),
    ]
    running = []
...
def collect_runtime_environment_snapshot() -> dict[str, Any]:
    ...
    if SERVICE_URL_MODE == "docker":
        configured = {
            ...
            "postgres": str(CONFIG["POSTGRES_DOCKER_CONTAINER"]),
        }
    ...
    if SERVICE_URL_MODE == "docker":
        result["docker"] = {
            "configured_containers": {
                ...
                "postgres": CONFIG["POSTGRES_DOCKER_CONTAINER"],
            },
...
class SentMessageRecord:
    ...
    client_request_started_at_epoch: float | None = None
    client_request_finished_at_epoch: float | None = None
    ...
    @property
    def db_query_fingerprints(self) -> list[dict[str, Any]]:
        if not self.ok:
            return []

        raw_samples = self.response_data.get(
        ...
        for raw_sample in raw_samples:
            ...
            fingerprint_id = str(
                raw_sample.get("fingerprint_id") or ""
            ).strip()
            fingerprint = str(
                raw_sample.get("fingerprint") or ""
            ).strip()
            operation = str(
                raw_sample.get("operation") or "OTHER"
            ).upper()
            stage = str(
                raw_sample.get("stage") or "unattributed"
            ).strip()

            raw_durations = raw_sample.get("durations_ms")
            ...
            if not fingerprint or not durations_ms:
                continue
            ...
            parsed.append(
                {
                    "fingerprint_id": fingerprint_id[:32],
                    "fingerprint": fingerprint[:800],
                    "operation": operation,
                    "stage": stage[:64] or "unattributed",
                    "many": bool(raw_sample.get("many")),
                    "calls": len(durations_ms),
...
async def send_one_message(
    ...
    *,
    ...
    async with semaphore:
        ...
        record.client_request_started_at_epoch = time.time()
        start = time.perf_counter()
        ...
        try:
            ...
            record.client_request_finished_at_epoch = time.time()
...
async def warm_active_user_clients(
    ...
    *,
    ...
    parallelism = max(1, int(CONFIG["CONNECTION_WARMUP_PARALLELISM"]))
    retry_attempts = max(1, int(CONFIG["CONNECTION_WARMUP_RETRY_ATTEMPTS"]))
    retry_delay_seconds = max(0.0, float(CONFIG["CONNECTION_WARMUP_RETRY_DELAY_SECONDS"]))
    semaphore = asyncio.Semaphore(parallelism)

    async def warm_one(pair: PairContext) -> tuple[bool, float, str | None, int]:
        ...
        for attempt in range(1, retry_attempts + 1):
            try:
                ...
                if 200 <= response.status_code < 300:
                    ...
                    return True, elapsed_ms, None, attempt
        ...
        elapsed_ms = (time.perf_counter() - total_started) * 1000
        return False, elapsed_ms, last_error, retry_attempts

    started = time.perf_counter()
    results = await asyncio.gather(*(warm_one(pair) for pair in pairs))
    elapsed_ms = (time.perf_counter() - started) * 1000
    failures = [
        {"pair_index": pair.index, "error": result[2], "attempts": result[3]}
        for pair, result in zip(pairs, results)
        if not result[0]
    ]
    latencies = [float(result[1]) for result in results]
    attempts_used = [int(result[3]) for result in results]
    retried_client_count = sum(1 for attempts in attempts_used if attempts > 1)
    summary = {
        "enabled": True,
        "phase": phase,
        ...
        "retried_client_count": retried_client_count,
        "max_attempts_used": max(attempts_used, default=0),
    ...
    if not failures and settle_seconds:
        await asyncio.sleep(settle_seconds)
    summary["settle_seconds"] = settle_seconds if not failures else 0.0
    return summary
...
# MYNA_SAFE_SQL_FINGERPRINT_V1
def summarize_db_query_fingerprints(
    records: list[SentMessageRecord],
    ...
    db_query_total_population_ms = 0.0

    for record in records:
        server_timings = record.server_timing_ms
        try:
            db_query_total_population_ms += float(
                server_timings.get("db_query_total_ms", 0.0)
            )
        except (TypeError, ValueError):
            pass

        for sample in record.db_query_fingerprints:
            key = (
                str(sample.get("fingerprint_id") or ""),
                str(sample.get("fingerprint") or ""),
                str(sample.get("operation") or "OTHER"),
                str(sample.get("stage") or "unattributed"),
                bool(sample.get("many")),
            )
            grouped.setdefault(key, []).extend(
                float(duration_ms)
                for duration_ms in (
                    sample.get("durations_ms") or []
                )
            )
    ...
    for (
        ...
        stage,
        many,
    ), durations_ms in grouped.items():
        if not durations_ms:
            continue

        latency = _latency_summary(durations_ms)
        total_ms = round(sum(durations_ms), 2)
        ...
        fingerprint_rows.append(
            {
                ...
                "percent_of_db_total": (
                    round(
                        (
                            total_ms
                            / db_query_total_population_ms
                        )
                        * 100,
                        2,
                    )
                    if db_query_total_population_ms > 0
                    else None
                ),
            }
        )
    ...
    fingerprint_rows.sort(
        key=lambda item: (
            ...
            str(item.get("fingerprint_id") or ""),
            str(item.get("stage") or ""),
        )
    )

    observed_total_ms = round(
        sum(
            float(item.get("total_ms") or 0.0)
    ...
    population = {
        ...
        "query_sample_count": sum(
            int(item.get("calls") or 0)
            for item in fingerprint_rows
        ),
        "fingerprint_stage_count": len(fingerprint_rows),
        "fingerprint_observed_total_ms": observed_total_ms,
        "db_query_total_population_ms": round(
            db_query_total_population_ms,
            2,
        ),
        "coverage_pct": (
        ...
        "safety": (
            "Normalized SQL text only; SQL params are never inspected, "
            "interpolated, serialized, hashed, or emitted."
        ),
    }

    return fingerprint_rows, population
...
def summarize_records(records: list[SentMessageRecord], concurrency: Any | None = None) -> dict[str, Any]:
    ...
    (
        db_query_fingerprints,
        db_query_fingerprint_population,
    ) = summarize_db_query_fingerprints(ok_records)
    ...
    return {
        ...
        "db_query_fingerprints": db_query_fingerprints,
        "db_query_fingerprint_population": (
            db_query_fingerprint_population
        ),
        "failed_samples": [
        ...
        "request_records": [
            {
                ...
                "semaphore_wait_ms": round(record.semaphore_wait_ms, 2) if record.semaphore_wait_ms is not None else None,
                "client_request_ms": round(record.client_request_ms, 2) if record.client_request_ms is not None else None,
                "client_request_started_at_epoch": (
                    round(record.client_request_started_at_epoch, 6)
                    if record.client_request_started_at_epoch is not None
                    else None
                ),
                "client_request_finished_at_epoch": (
                    round(record.client_request_finished_at_epoch, 6)
                    if record.client_request_finished_at_epoch is not None
                    else None
                ),
...
def validate_realtime_outbox_drain(pairs: list[PairContext]) -> dict[str, Any]:
    if not CONFIG["OUTBOX_DRAIN_ENABLED"]:
        return {"enabled": False, "success": True, "message": "Outbox drain validation disabled."}
    ...
    if not (root / "manage.py").exists():
        return {
            "enabled": True,
            "success": False,
            "message": f"manage.py not found at {root}; cannot validate outbox drain.",
        }
    ...
    try:
        ...
        device_ids = sorted(
            {
                ...
                for pair in pairs
                for device_id in (
                    pair.sender_device.device_id,
                    pair.recipient_device.device_id,
                )
                if device_id
            }
        )
        target_groups = [f"user.{user_id}" for user_id in user_ids] + [
            f"device.{device_id}" for device_id in device_ids
        ]
        if not target_groups:
            return {"enabled": True, "success": True, "message": "No benchmark outbox targets."}

        timeout_seconds = max(0.0, float(CONFIG["OUTBOX_DRAIN_TIMEOUT_SECONDS"]))
        poll_seconds = max(0.05, float(CONFIG["OUTBOX_DRAIN_POLL_SECONDS"]))
        ...
        while True:
            ...
            queryset = RealtimeOutboxEvent.objects.filter(target_group__in=target_groups)
            counts = {status: 0 for status, _label in RealtimeOutboxEvent.Status.choices}
            # Aggregate using values/Count on every supported DB backend.
            ...
            active = sum(counts.get(key, 0) for key in ("pending", "processing", "failed"))
            ...
            if dead:
                return {
                    ...
                    "message": f"Outbox contains {dead} dead-letter event(s).",
                }
            if observed_total > 0 and active == 0:
                return {
                    "enabled": True,
                    "success": True,
                    "counts": counts,
                    "observed_total": observed_total,
                    "message": "Benchmark outbox events drained.",
                }
            if time.monotonic() >= deadline:
                return {
                    ...
                    "counts": last_counts,
                    "observed_total": observed_total,
                    "message": f"Outbox did not drain within {timeout_seconds} seconds.",
                }
...
def cleanup_messenger_with_django(pairs: list[PairContext]) -> dict[str, Any]:
    if not CONFIG["CLEANUP_MESSENGER_DJANGO"]:
        return {"enabled": False, "success": True, "message": "Messenger Django cleanup disabled."}
    ...
    if not (root / "manage.py").exists():
        return {
            ...
            "failed_samples": [{"reason": f"manage.py not found at {root}."}],
            "message": f"manage.py not found at {root}.",
        }
    ...
    try:
        ...
        device_ids = sorted({str(item) for item in device_ids if item})
        ...
        if not user_ids and not device_ids:
            return {
                ...
                "remaining": {},
                "failed_samples": [],
                "message": "No Messenger IDs to clean.",
            }
        ...
        def target_ids() -> tuple[list[Any], list[Any]]:
            room_ids = list(
                RoomMember.objects.filter(user_id__in=user_ids)
                .values_list("room_id", flat=True)
                .distinct()
            )
            message_ids = list(
        ...
        def remaining_counts(
            ...
            attachment_filter = (
                Q(uploader_user_id__in=user_ids)
                | Q(uploader_device_id__in=device_ids)
                | Q(attached_room_id__in=room_ids)
                | Q(attached_message_id__in=message_ids)
            )
            return {
                "message_receipts": MessageReceipt.objects.filter(
                    Q(message_id__in=message_ids)
                    | Q(recipient_user_id__in=user_ids)
                    | Q(recipient_device_id__in=device_ids)
                ).count(),
                "message_recovery_envelopes": MessageRecoveryEnvelope.objects.filter(
                    Q(message_id__in=message_ids)
                    | Q(recovery_owner_user_id__in=user_ids)
                ).count(),
                "message_key_envelopes": MessageKeyEnvelope.objects.filter(
                    Q(message_id__in=message_ids)
                    | Q(recipient_user_id__in=user_ids)
                    | Q(recipient_device_id__in=device_ids)
                ).count(),
                "direct_message_receipt_decisions": DirectMessageReceiptDecision.objects.filter(
                    Q(message_id__in=message_ids)
                    | Q(sender_user_id__in=user_ids)
                    | Q(recipient_user_id__in=user_ids)
                ).count(),
                "encrypted_attachments": EncryptedAttachment.objects.filter(
                    attachment_filter
                ).count(),
                "messages": Message.objects.filter(id__in=message_ids).count(),
                "direct_contact_states": DirectContactState.objects.filter(
                    Q(owner_user_id__in=user_ids)
                    | Q(contact_user_id__in=user_ids)
                ).count(),
                "room_members": RoomMember.objects.filter(
                    Q(user_id__in=user_ids) | Q(room_id__in=room_ids)
                ).count(),
                "rooms": Room.objects.filter(id__in=room_ids).count(),
                "one_time_prekeys": OneTimePreKey.objects.filter(
                    device_id__in=device_ids
                ).count(),
                "devices": Device.objects.filter(
                    Q(user_id__in=user_ids) | Q(id__in=device_ids)
                ).count(),
                "contact_policies": ContactDeliveryPolicy.objects.filter(
                    Q(owner_user_id__in=user_ids)
                    | Q(target_user_id__in=user_ids)
                ).count(),
                "recovery_bundles": RecoveryBundle.objects.filter(
                    user_id__in=user_ids
                ).count(),
                "realtime_tickets": RealtimeTicket.objects.filter(
                    Q(user_id__in=user_ids) | Q(device_id__in=device_ids)
                ).count(),
                "realtime_outbox_events": RealtimeOutboxEvent.objects.filter(
                    target_group__in=target_groups
                ).count(),
            }

        def delete_target_data() -> dict[str, Any]:
            room_ids, message_ids = target_ids()

            with transaction.atomic():
                outbox_deleted, _ = RealtimeOutboxEvent.objects.filter(
                    target_group__in=target_groups
                ).delete()
                tickets_deleted, _ = RealtimeTicket.objects.filter(
                    Q(user_id__in=user_ids) | Q(device_id__in=device_ids)
                ).delete()
                receipts_deleted, _ = MessageReceipt.objects.filter(
                    Q(message_id__in=message_ids)
                    | Q(recipient_user_id__in=user_ids)
                    | Q(recipient_device_id__in=device_ids)
                ).delete()
                recovery_envelopes_deleted, _ = MessageRecoveryEnvelope.objects.filter(
                    Q(message_id__in=message_ids)
                    | Q(recovery_owner_user_id__in=user_ids)
                ).delete()
                key_envelopes_deleted, _ = MessageKeyEnvelope.objects.filter(
                    Q(message_id__in=message_ids)
                    | Q(recipient_user_id__in=user_ids)
                    | Q(recipient_device_id__in=device_ids)
                ).delete()
                receipt_decisions_deleted, _ = DirectMessageReceiptDecision.objects.filter(
                    Q(message_id__in=message_ids)
                    | Q(sender_user_id__in=user_ids)
                    | Q(recipient_user_id__in=user_ids)
                ).delete()
                attachments_deleted, _ = EncryptedAttachment.objects.filter(
                    Q(uploader_user_id__in=user_ids)
                    | Q(uploader_device_id__in=device_ids)
                    | Q(attached_room_id__in=room_ids)
                    | Q(attached_message_id__in=message_ids)
                ...
                contact_states_deleted, _ = DirectContactState.objects.filter(
                    Q(owner_user_id__in=user_ids)
                    | Q(contact_user_id__in=user_ids)
                ).delete()
                rooms_deleted, _ = Room.objects.filter(
                    id__in=room_ids
                ).delete()
                policies_deleted, _ = ContactDeliveryPolicy.objects.filter(
                    Q(owner_user_id__in=user_ids)
                    | Q(target_user_id__in=user_ids)
                ).delete()
                prekeys_deleted, _ = OneTimePreKey.objects.filter(
                    device_id__in=device_ids
                ).delete()
                recovery_deleted, _ = RecoveryBundle.objects.filter(
                    user_id__in=user_ids
                ).delete()
                devices_deleted, _ = Device.objects.filter(
                    Q(user_id__in=user_ids) | Q(id__in=device_ids)
                ).delete()
            ...
            deleted = {
                ...
                "direct_message_receipt_decisions": receipt_decisions_deleted,
                "encrypted_attachments": attachments_deleted,
                "messages_and_cascades": messages_deleted,
                "direct_contact_states": contact_states_deleted,
                "rooms_and_members": rooms_deleted,
                "devices_and_cascades": devices_deleted,
        ...
        delay_seconds = max(
            0.0,
            float(CONFIG["CLEANUP_DB_RETRY_DELAY_SECONDS"]),
        )
        cleanup_attempts = 0
        deleted_result: dict[str, Any] | None = None

        retryable_operational_fragments = (
        ...
        for attempt in range(1, attempts + 1):
            ...
            try:
                ...
                if not any(
                    fragment in repr(exc).lower()
                    for fragment in retryable_operational_fragments
                ):
                    raise
                connections.close_all()
                if attempt >= attempts:
        ...
        if deleted_result is None:
            raise RuntimeError("Messenger cleanup retry loop exhausted.")
        ...
        close_old_connections()
        remaining = remaining_counts(room_ids, message_ids)
        remaining_count = sum(int(value) for value in remaining.values())
        deleted = deleted_result["deleted"]
        deleted_count = sum(int(value) for value in deleted.values())
        failed_samples = [
        ...
        return {
            ...
            "message": (
                "Messenger benchmark cleanup verified: no target rows remain."
                if success
                else f"Messenger benchmark cleanup incomplete: {remaining_count} target row(s) remain."
            ),
        }
        ...
        return {
            ...
            "deleted_count": 0,
            "remaining_count": None,
            "failed_samples": [{"reason": repr(exc)}],
            "message": repr(exc),
            "traceback": traceback.format_exc(),
        }
...
def verify_identity_cleanup_in_postgres(users: list[TestUser]) -> dict[str, Any]:
    database_url = IDENTITY_DATABASE_URL
    usernames = sorted({str(user.username) for user in users if user.username})
    if not usernames:
        return {
            "available": True,
            "success": True,
            "remaining_count": 0,
            "remaining_usernames_sample": [],
            "message": "No Identity benchmark usernames required verification.",
        }
    if not database_url:
        return {
            "available": False,
            ...
            "remaining_count": None,
            "remaining_usernames_sample": [],
            "message": "IDENTITY_DATABASE_URL is missing; post-delete Identity verification could not run.",
        }

    connect_url = database_url.replace("postgresql+psycopg://", "postgresql://", 1)
    connect_url = connect_url.replace("postgres+psycopg://", "postgresql://", 1)
    try:
        import psycopg
        ...
        with psycopg.connect(connect_url, connect_timeout=10) as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT username FROM users WHERE username = ANY(%s) ORDER BY username",
                    (usernames,),
                )
                remaining_usernames = [str(row[0]) for row in cursor.fetchall()]
        return {
            "available": True,
            "success": len(remaining_usernames) == 0,
            "remaining_count": len(remaining_usernames),
            "remaining_usernames_sample": remaining_usernames[:20],
            "database_host": url_hostname(connect_url),
            "message": (
                "Identity benchmark cleanup verified: no target users remain."
                if not remaining_usernames
                else f"Identity cleanup verification found {len(remaining_usernames)} benchmark users still present."
            ),
        }
    except Exception as exc:
        return {
            ...
            "remaining_usernames_sample": [],
            "database_host": url_hostname(connect_url),
            "message": "Identity PostgreSQL post-delete verification failed.",
            "error": repr(exc),
            "traceback": traceback.format_exc(),
        }
...
async def cleanup_identity_users(client: httpx.AsyncClient, pairs: list[PairContext]) -> dict[str, Any]:
    ...
    for index, user in enumerate(users, start=1):
        ...
        if delay > 0 and index < len(users):
            await asyncio.sleep(delay)
    deletion_success = all(item.get("deleted") for item in results)
    verification = await asyncio.to_thread(verify_identity_cleanup_in_postgres, users)
    cleanup_success = bool(deletion_success and verification.get("success", False))
    return {
        "enabled": True,
        "success": cleanup_success,
        "deleted_count": sum(1 for item in results if item.get("deleted")),
        "total_user_count": len(users),
        "remaining_count": verification.get("remaining_count"),
        "remaining": {"users": verification.get("remaining_count")},
...
async def run() -> int:
    ...
    report: dict[str, Any] = {
        ...
        "effective_runtime_config": {
            "benchmark_http": {
                ...
                "MYNA_CONNECTION_WARMUP_PARALLELISM": CONFIG["CONNECTION_WARMUP_PARALLELISM"],
                "MYNA_CONNECTION_WARMUP_RETRY_ATTEMPTS": CONFIG["CONNECTION_WARMUP_RETRY_ATTEMPTS"],
                "MYNA_CONNECTION_WARMUP_RETRY_DELAY_SECONDS": CONFIG["CONNECTION_WARMUP_RETRY_DELAY_SECONDS"],
    ...
    try:
        async with make_benchmark_http_client() as setup_client:
            await add_postgres_telemetry(report, "initial_pre_benchmark")
            ...
            per_level: list[dict[str, Any]] = report["benchmark"]["per_level"]
            ...
            async with AsyncExitStack() as measured_client_stack:
                ...
                await add_postgres_telemetry(report, "before_connection_warmup")
                ...
                await add_postgres_telemetry(report, "after_connection_warmup")
                report["connection_warmup"] = initial_connection_warmup
                if not initial_connection_warmup.get("success", False):
                    raise RuntimeError(
                        "Initial active-user connection warmup failed after "
                        f"{initial_connection_warmup.get('retry_attempts')} attempts with "
                        f"bounded parallelism={initial_connection_warmup.get('parallelism')} for "
                        f"{initial_connection_warmup.get('failure_count')} clients: "
                        f"{initial_connection_warmup.get('failed_samples', [])[:5]}"
                    )
                ...
                for level in levels:
                    ...
                    if not prelevel_warmup.get("success", False):
                        report.setdefault("connection_warmup_failures", []).append(
                            prelevel_warmup
                        )
                        raise RuntimeError(
                            f"Pre-level active-user connection warmup failed "
                            f"for concurrency={level} "
                            f"after {prelevel_warmup.get('retry_attempts')} attempts with "
                            f"bounded parallelism={prelevel_warmup.get('parallelism')} for "
                            f"{prelevel_warmup.get('failure_count')} clients: "
                            f"{prelevel_warmup.get('failed_samples', [])[:5]}"
                        )

                    # A retry during pre-level connection warmup is a signal that one or
                    # more persistent client connections were no longer immediately usable.
                    # This can happen around Gunicorn worker recycling. Allow the connection
                    # transition to settle, then perform a second unmeasured verification
                    # before releasing the measured concurrency burst.
                    if int(prelevel_warmup.get("retried_client_count") or 0) > 0:
                        await asyncio.sleep(6.0)

                        stabilization_warmup = await warm_active_user_clients(
                            clients_by_pair_index,
                            pairs=selected_pairs,
                            phase=f"prelevel_active_user_stabilization_{level}",
                        )

                        if not stabilization_warmup.get("success", False):
                            report.setdefault("connection_warmup_failures", []).append(
                                stabilization_warmup
                            )
                            raise RuntimeError(
                                f"Pre-level active-user stabilization failed "
                                f"for concurrency={level} "
                                f"after {stabilization_warmup.get('retry_attempts')} attempts with "
                                f"bounded parallelism={stabilization_warmup.get('parallelism')} for "
                                f"{stabilization_warmup.get('failure_count')} clients: "
                                f"{stabilization_warmup.get('failed_samples', [])[:5]}"
                            )
                    ...
                    await add_postgres_telemetry(report, f"before_concurrency_{level}")
                    ...
                    await add_postgres_telemetry(report, f"after_concurrency_{level}")
                    per_level.append(level_summary)
                    report["benchmark"]["analysis"] = analyze_benchmark_levels(per_level)
        ...
        partial_levels = (report.get("benchmark") or {}).get("per_level") or []
        if isinstance(report.get("benchmark"), dict):
            report["benchmark"]["analysis"] = analyze_benchmark_levels(partial_levels)
            report["benchmark"]["levels_completed"] = [level.get("concurrency") for level in partial_levels]
        report["docker_stats"] = docker_stats_sampler.summary()
    finally:
        await docker_stats_sampler.stop()
        set_api_phase("cleanup")
        log_progress("Validating realtime outbox drain before cleanup...")
        outbox_drain = await asyncio.to_thread(validate_realtime_outbox_drain, pairs)
        report["realtime_outbox_drain_validation"] = outbox_drain
        if outbox_drain.get("enabled") and not outbox_drain.get("success", False):
            report["passed"] = False

        log_progress("Starting cleanup...")
        await add_postgres_telemetry(report, "before_cleanup")
        ...
        if not report["cleanup_success"]:
            report["passed"] = False
        await add_postgres_telemetry(report, "after_cleanup")
