...
def target_summary_rows(report: dict[str, Any], concurrency: int | None) -> list[list[Any]]:
    ...
    structural_server_avg = (
        metric(source, "server_structural_total_ms", "avg") or server_avg
    )
    ...
    structural_outside_view_avg = (
        metric(source, "server_structural_outside_view_ms", "avg")
        or outside_view_avg
    )
    ...
    return [
        ...
        ["server_structural_total_avg_ms", structural_server_avg],
        ["server_boundary_source_delta_avg_ms", metric(source, "server_boundary_source_delta_ms", "avg")],
        ["server_pre_view_avg_ms", pre_view_avg],
        ["server_pre_view_percent_of_server", percent_of(pre_view_avg, structural_server_avg)],
        ...
        ["server_post_view_percent_of_server", percent_of(post_view_avg, structural_server_avg)],
        ["server_post_view_percent_of_client", percent_of(post_view_avg, client_avg)],
        ["server_outside_view_avg_ms", outside_view_avg],
        ["server_structural_outside_view_avg_ms", structural_outside_view_avg],
        ["server_outside_view_percent_of_client", percent_of(structural_outside_view_avg, client_avg)],
        ...
        ["db_transaction_enter_avg_ms", metric(source, "db_transaction_enter_ms", "avg")],
        ["db_transaction_body_avg_ms", metric(source, "db_transaction_body_ms", "avg")],
        ["db_transaction_commit_avg_ms", metric(source, "db_transaction_commit_ms", "avg")],
        ["db_transaction_rollback_avg_ms", metric(source, "db_transaction_rollback_ms", "avg")],
        ["db_transaction_exit_avg_ms", metric(source, "db_transaction_exit_ms", "avg")],
...
def latency_decomposition_rows(row: dict[str, Any] | None) -> list[list[Any]]:
    ...
    structural_server_avg = (
        metric(row, "server_structural_total_ms", "avg") or server_avg
    )
    ...
    items = [
        ...
        ("SERVER TOTAL (Gunicorn access boundary)", "server_total_ms", server_avg, client_avg, client_avg),
        ("SERVER STRUCTURAL TOTAL (overlapping observation)", "server_structural_total_ms", structural_server_avg, None, client_avg),
        ("server pre-view", "server_pre_view_ms", server_pre_view_avg, structural_server_avg, client_avg),
        ("VIEW TOTAL", "view_total_ms", view_avg, structural_server_avg, client_avg),
        ("server post-view", "server_post_view_ms", server_post_view_avg, structural_server_avg, client_avg),
        ...
        ("transaction enter", "db_transaction_enter_ms", metric(row, "db_transaction_enter_ms", "avg"), recovery_base_send_avg, client_avg),
        ("transaction body", "db_transaction_body_ms", metric(row, "db_transaction_body_ms", "avg"), recovery_base_send_avg, client_avg),
        ("transaction commit", "db_transaction_commit_ms", metric(row, "db_transaction_commit_ms", "avg"), recovery_base_send_avg, client_avg),
        ("transaction rollback", "db_transaction_rollback_ms", metric(row, "db_transaction_rollback_ms", "avg"), recovery_base_send_avg, client_avg),
...
def server_boundary_decomposition_rows(row: dict[str, Any] | None) -> list[list[Any]]:
    if not row:
        return []
    server_avg = (
        metric(row, "server_structural_total_ms", "avg")
        or metric(row, "server_total_ms", "avg")
        or metric(row, "gunicorn_request_ms", "avg")
    )
    client_avg = metric(row, "client_latency_ms", "avg")
    regions = [
...
def server_boundary_cross_check_rows(row: dict[str, Any] | None) -> list[list[Any]]:
    ...
    gunicorn_total = metric(row, "server_total_ms", "avg") or metric(
        row, "gunicorn_request_ms", "avg"
    )
    structural_total = metric(row, "server_structural_total_ms", "avg")
    compatibility_outside = metric(row, "server_outside_view_ms", "avg") or metric(
        row, "gunicorn_outside_view_ms", "avg"
    )
    structural_outside = metric(
        row, "server_structural_outside_view_ms", "avg"
    )
    rows = [
        ["server_total_ms", gunicorn_total, "Gunicorn access-log request duration; used for client_outside_server_ms"],
        ["server_structural_total_ms", structural_total, "request-local WSGI structural parent for pre-view + view + post-view"],
        ["server_boundary_source_delta_ms", metric(row, "server_boundary_source_delta_ms", "avg"), "Gunicorn access duration minus request-local WSGI structural total"],
        ["server_outside_view_ms", compatibility_outside, "compatibility delta: Gunicorn server_total_ms - view_total_ms"],
        ["server_structural_outside_view_ms", structural_outside, "structural parent: server_structural_total_ms - view_total_ms"],
        ["server_pre_view_ms + server_post_view_ms", (num(pre) or 0) + (num(post) or 0) if pre is not None and post is not None else None, "non-overlapping measured structural outside-view split"],
        ["server_boundary_reconciliation_delta_ms", metric(row, "server_boundary_reconciliation_delta_ms", "avg"), "request-local structural total - (pre-view + view + post-view)"],
        ["server_outside_view_reconciliation_delta_ms", metric(row, "server_outside_view_reconciliation_delta_ms", "avg"), "structural outside-view - (pre-view + post-view)"],
    ]
    return [item for item in rows if item[1] is not None]
...
def outside_view_share_rows(row: dict[str, Any] | None) -> list[list[Any]]:
    if not row:
        return []
    outside = (
        metric(row, "server_structural_outside_view_ms", "avg")
        or metric(row, "server_outside_view_ms", "avg")
        or metric(row, "gunicorn_outside_view_ms", "avg")
    )
    rows = []
...
def post_view_decomposition_rows(row: dict[str, Any] | None) -> list[list[Any]]:
    ...
    wsgi_regions = [
        ("view exit -> DRF finalize entry", "post_view_to_drf_finalize_entry_ms"),
        ("DRF finalize_response", "drf_finalize_response_ms"),
        ("DRF finalize exit -> WSGI start_response", "drf_finalize_to_wsgi_start_response_ms"),
        ("WSGI start_response -> Django WSGI app return", "wsgi_start_response_to_app_return_ms"),
        ("Django WSGI app return -> response iteration start", "wsgi_app_return_to_response_iter_start_ms"),
        ("WSGI response iteration / Gunicorn writes", "wsgi_response_iter_ms"),
        ("WSGI response complete -> Gunicorn handle_request return", "wsgi_after_response_complete_ms"),
        ("post-view unattributed", "post_view_unattributed_ms"),
    ]
    asgi_regions = [
    ...
    regions = (
        wsgi_regions
        if any(metric(row, name, "avg") is not None for _, name in wsgi_regions)
        else asgi_regions
    )
    rows = []
    for label, metric_name in regions:
...
def pre_view_structural_metric_names(row: dict[str, Any] | None) -> list[str]:
    ...
    entry_region = (
        ...
        if metric(row, "pre_view_wsgi_to_django_ms", "avg") is not None
        else "pre_view_asgi_to_django_ms"
    )
    return [
        entry_region,
        *django_regions,
...
def transaction_timing_rows(row: dict[str, Any] | None) -> list[list[Any]]:
    if not row:
        return []
    regions = [
        ("transaction enter", "db_transaction_enter_ms"),
        ("transaction body", "db_transaction_body_ms"),
        ("transaction commit", "db_transaction_commit_ms"),
        ("transaction rollback", "db_transaction_rollback_ms"),
        ("transaction exit", "db_transaction_exit_ms"),
    ]
    rows = []
    for label, name in regions:
        avg = metric(row, name, "avg")
...
def query_fingerprint_rows(
    row: dict[str, Any] | None,
) -> list[list[Any]]:
    if not row:
        return []

    fingerprints = row.get("db_query_fingerprints")
...
def append_database_observation_tables(lines: list[str], row: dict[str, Any] | None) -> None:
    ...
    fingerprint_rows = query_fingerprint_rows(row)
    lines.extend(["", "**Query Fingerprint Timing**", ""])
    lines.append(
        "Fingerprints contain normalized SQL structure only. "
        "SQL parameters are never inspected, interpolated, serialized, "
        "hashed, or emitted."
    )
    lines.append("")

    population = (
        row.get("db_query_fingerprint_population")
    ...
    if isinstance(population, dict) and population:
        lines.extend(
            markdown_table(
                ["metric", "value"],
                [
                    ["request_count", population.get("request_count")],
                    ["query_sample_count", population.get("query_sample_count")],
                    ["fingerprint_stage_count", population.get("fingerprint_stage_count")],
                    ["fingerprint_observed_total_ms", population.get("fingerprint_observed_total_ms")],
                    ["db_query_total_population_ms", population.get("db_query_total_population_ms")],
                    ["coverage_pct", population.get("coverage_pct")],
                ],
            )
        )
        lines.append("")
    ...
    if fingerprint_rows:
        ...
        lines.append(
            "No target-concurrency query fingerprint samples were available."
        )
...
def runtime_services(report: dict[str, Any]) -> dict[str, Any]:
    ...
    # The runtime-config capture may contain the Messenger settings dictionary
    # directly instead of a service-name -> {container, env} mapping. Normalize
    # that flat form so report rendering remains backward-compatible and robust.
    service_shape = any(
        isinstance(value, dict)
        and ("container" in value or "env" in value)
        for value in raw_services.values()
    )
    if raw_services and not service_shape:
        return {
            "messenger": {
                "container": None,
                "env": raw_services,
...
def messenger_http_mode(report: dict[str, Any]) -> str:
    configured = str(
        messenger_env_from_report(report).get("http_server_mode") or ""
    ).strip().lower()
    if configured in {"wsgi", "asgi"}:
        return configured

    observed_modes: set[str] = set()
    for level in (report.get("request_gap_analysis") or {}).get("per_level") or []:
        for raw_mode in level.get("server_timing_modes") or []:
            mode = str(raw_mode or "").strip().lower()
            if mode in {"wsgi", "asgi"}:
...
def server_boundary_source(report: dict[str, Any]) -> str:
    mode = messenger_http_mode(report)
    if mode == "wsgi":
        return (
            "Gunicorn WSGI request duration_us access-log boundary with "
            "request-local WSGI/DRF boundary correlation"
        )
    if mode == "asgi":
        return "benchmark ASGI application wrapper boundary"
    return "unknown server boundary source"
    ...
    *,
    ...
    for key, value in config.items():
        if not include_source_paths and key in {"REPORT_DIR"}:
            continue
        label = (
            "input.RUNNER_INTERNAL_DOCKER_STATS_ENABLED"
            if key == "DOCKER_STATS_ENABLED"
            else f"input.{key}"
        )
    ...
    rows.extend(
        [
            ...
            ["derived.messenger_http_mode", messenger_http_mode(report)],
            ["derived.server_boundary_source", server_boundary_source(report)],
            ["host_docker_stats.available", ((report.get("request_gap_analysis") or {}).get("host_docker_stats") or {}).get("available")],
            ["host_docker_stats.sample_count", ((report.get("request_gap_analysis") or {}).get("host_docker_stats") or {}).get("sample_count")],
        ]
    )
    return rows
...
def success_failure_rows(report: dict[str, Any], metadata: dict[str, Any] | None = None) -> list[list[Any]]:
    ...
    rows = [
        ...
        ["fatal_error_present", bool(report.get("fatal_error"))],
        ["traceback_present", bool(report.get("traceback"))],
        ["launcher_version", (report.get("benchmark_launcher") or {}).get("version")],
        ["cleanup_success", report.get("cleanup_success")],
        ["outbox_worker_enabled", drain.get("enabled")],
        ["outbox_drain_succeeded", drain.get("success")],
        ["outbox_drain_observed_total", drain.get("observed_total")],
        ["outbox_drain_counts", drain.get("counts")],
        ["outbox_drain_message", drain.get("message")],
    ...
    for service_name, result in (report.get("cleanup") or {}).items():
        if isinstance(result, dict):
            ...
            if deleted_count is None and isinstance(deleted, dict):
                deleted_count = sum(
                    int(value)
            ...
            if remaining_count is None and isinstance(remaining, dict):
                remaining_count = sum(
                    int(value)
            ...
            if failed_samples is None and isinstance(remaining, dict):
                failed_samples = [
                    {"model": model_name, "remaining_count": count}
                    for model_name, count in remaining.items()
            ...
            rows.append([f"cleanup.{service_name}.deleted_count", deleted_count])
            rows.append([f"cleanup.{service_name}.remaining_count", remaining_count])
            rows.append([f"cleanup.{service_name}.deleted", deleted])
            rows.append([f"cleanup.{service_name}.remaining", remaining])
            rows.append([f"cleanup.{service_name}.failed_samples", failed_samples])
            rows.append([f"cleanup.{service_name}.message", result.get("message")])
...
def accurate_timing_table_rows(report: dict[str, Any]) -> list[list[Any]]:
    ...
    for level in levels:
        ...
        prelevel_warmup = level.get("prelevel_connection_warmup") or {}
        ...
        rows.append(
            [
                ...
                prelevel_warmup.get("success_count"),
                prelevel_warmup.get("failure_count"),
                prelevel_warmup.get("retried_client_count"),
                prelevel_warmup.get("max_attempts_used"),
                metric(prelevel_warmup, "latency_ms", "avg"),
                ...
                metric(source, "server_structural_total_ms", "avg"),
                metric(source, "server_boundary_source_delta_ms", "avg"),
                ...
                metric(source, "db_transaction_enter_ms", "avg"),
                metric(source, "db_transaction_body_ms", "avg"),
                metric(source, "db_transaction_commit_ms", "avg"),
                metric(source, "db_transaction_rollback_ms", "avg"),
                metric(source, "db_transaction_exit_ms", "avg"),
                metric(source, "post_view_unattributed_ms", "avg"),
                ...
                docker_stats.get("_container"),
                docker_stats.get("_sample_count"),
                docker_stats.get("_sample_source"),
                source.get("host_docker_window_source"),
                cpu.get("min"),
                cpu.get("avg"),
                cpu.get("max"),
                cpu.get("p95"),
                ...
                mem.get("max"),
                mem.get("p95"),
                (docker_stats.get("mem_usage_mb") or {}).get("avg"),
                (docker_stats.get("mem_usage_mb") or {}).get("max"),
                (docker_stats.get("net_rx_mb") or {}).get("delta"),
                (docker_stats.get("net_tx_mb") or {}).get("delta"),
                (docker_stats.get("block_read_mb") or {}).get("delta"),
                (docker_stats.get("block_write_mb") or {}).get("delta"),
                (docker_stats.get("pids") or {}).get("avg"),
                (docker_stats.get("pids") or {}).get("max"),
            ]
        )
    return rows
...
def accurate_timing_headers() -> list[str]:
    return [
        ...
        "success",
        "failure",
        "prelevel_warmup_success",
        "prelevel_warmup_failure",
        "prelevel_warmup_retried_clients",
        "prelevel_warmup_max_attempts",
        "prelevel_warmup_avg_ms",
        "msg/s",
        "client_avg_ms",
        "client_p50_ms",
        "client_p95_ms",
        ...
        "server_avg_ms",
        "server_p95_ms",
        "server_structural_total_avg_ms",
        "server_boundary_source_delta_avg_ms",
        "server_pre_view_avg_ms",
        "view_avg_ms",
        "view_p95_ms",
        "server_post_view_avg_ms",
        "server_outside_view_avg_ms",
        "server_structural_outside_view_avg_ms",
        ...
        "db_transaction_enter_avg_ms",
        "db_transaction_body_avg_ms",
        "db_transaction_commit_avg_ms",
        "db_transaction_rollback_avg_ms",
        "db_transaction_exit_avg_ms",
        "post_view_unattributed_avg_ms",
        "db_select_count_avg",
        "db_insert_count_avg",
        "db_update_count_avg",
        "db_delete_count_avg",
        ...
        "docker_mem_pct_max",
        "docker_mem_pct_p95",
        "docker_mem_usage_avg_mb",
        "docker_mem_usage_max_mb",
        "docker_net_rx_delta_mb",
        "docker_net_tx_delta_mb",
        "docker_block_read_delta_mb",
        "docker_block_write_delta_mb",
        "docker_pids_avg",
        "docker_pids_max",
    ]
...
def docker_usage_rows(report: dict[str, Any]) -> list[list[Any]]:
    ...
    for container, stats in host_stats.items():
        ...
        mem_usage = stats.get("mem_usage_mb") or {}
        net_rx = stats.get("net_rx_mb") or {}
        net_tx = stats.get("net_tx_mb") or {}
        block_read = stats.get("block_read_mb") or {}
        block_write = stats.get("block_write_mb") or {}
        pids = stats.get("pids") or {}
        rows.append(
            [
                container,
                stats.get("sample_count"),
                cpu.get("min"), cpu.get("avg"), cpu.get("max"), cpu.get("p95"),
                mem.get("min"), mem.get("avg"), mem.get("max"), mem.get("p95"),
                mem_usage.get("avg"), mem_usage.get("max"),
                net_rx.get("delta"), net_tx.get("delta"),
                block_read.get("delta"), block_write.get("delta"),
                pids.get("avg"), pids.get("max"),
            ]
        )
    return rows


def benchmark_harness_rows(report: dict[str, Any]) -> list[list[Any]]:
    launcher = report.get("benchmark_launcher") or {}
    preflight = report.get("preflight") or {}
    lifecycle = report.get("httpx_client_lifecycle") or {}
    setup = report.get("setup") or {}
    setup_diag = setup.get("failure_diagnostics") or {}
    connection_warmup = report.get("connection_warmup") or {}
    warmup = report.get("warmup") or {}
    drain = report.get("realtime_outbox_drain_validation") or {}
    gap = report.get("request_gap_analysis") or {}
    queue_summary = report.get("gunicorn_gthread_queue_analysis") or {}
    queue_sample_count = queue_summary.get("sample_count")
    if queue_sample_count is None:
        queue_sample_count = sum(
            int(item.get("sample_count") or 0)
            for item in queue_summary.get("per_level") or []
        )
    queue_expected_count = queue_summary.get("expected_request_count")
    if queue_expected_count is None:
        queue_expected_count = sum(
            int(item.get("expected_request_count") or 0)
            for item in queue_summary.get("per_level") or []
        )
    queue_missing_count = queue_summary.get("missing_sample_count")
    if queue_missing_count is None and queue_expected_count is not None:
        queue_missing_count = max(
            0,
            int(queue_expected_count or 0) - int(queue_sample_count or 0),
        )
    benchmark = report.get("benchmark") or {}
    warmup_failures = report.get("connection_warmup_failures") or []
    last_warmup_failure = warmup_failures[-1] if warmup_failures else {}
    return [
        ["launcher.version", launcher.get("version")],
        ["launcher.failure_readme_enabled", launcher.get("failure_readme_enabled")],
        ["launcher.intermediate_artifacts_deleted_after_readme", launcher.get("intermediate_artifacts_deleted_after_readme")],
        ["host_docker_stats_enabled", launcher.get("host_docker_stats_enabled")],
        ["host_docker_stats_sample_rows", launcher.get("host_docker_stats_sample_rows")],
        ["host_docker_stats_sampler_mode", launcher.get("host_docker_stats_sampler_mode")],
        ["server_boundary_source", server_boundary_source(report)],
        ["preflight.ok", preflight.get("ok")],
        ["preflight.identity_status", (preflight.get("identity") or {}).get("status_code")],
        ["preflight.messenger_status", (preflight.get("messenger") or {}).get("status_code")],
        ["httpx.measured_clients", lifecycle.get("measured_clients")],
        ["httpx.measured_start_model", lifecycle.get("measured_start_model")],
        ["httpx.per_level_connection_refresh", lifecycle.get("per_level_connection_refresh")],
        ["setup_pairs_requested", setup_diag.get("pair_count_requested")],
        ["setup_pairs_successful", setup_diag.get("pair_count_successful")],
        ["setup_pairs_failed", setup_diag.get("failed_count")],
        ["initial_connection_warmup.enabled", connection_warmup.get("enabled")],
        ["initial_connection_warmup.success", connection_warmup.get("success")],
        ["initial_connection_warmup.client_count", connection_warmup.get("client_count")],
        ["initial_connection_warmup.parallelism", connection_warmup.get("parallelism")],
        ["initial_connection_warmup.retry_attempts", connection_warmup.get("retry_attempts")],
        ["initial_connection_warmup.retried_client_count", connection_warmup.get("retried_client_count")],
        ["initial_connection_warmup.max_attempts_used", connection_warmup.get("max_attempts_used")],
        ["initial_connection_warmup.success_count", connection_warmup.get("success_count")],
        ["initial_connection_warmup.failure_count", connection_warmup.get("failure_count")],
        ["initial_connection_warmup.latency_ms", connection_warmup.get("latency_ms")],
        ["room_creation_warmup.success_count", warmup.get("success_count")],
        ["room_creation_warmup.failure_count", warmup.get("failure_count")],
        ["prelevel_warmup_failure_count", len(warmup_failures)],
        ["last_prelevel_warmup_failure.phase", last_warmup_failure.get("phase")],
        ["last_prelevel_warmup_failure.failure_count", last_warmup_failure.get("failure_count")],
        ["last_prelevel_warmup_failure.failed_samples", last_warmup_failure.get("failed_samples")],
        ["benchmark.levels_requested", benchmark.get("levels_requested") or (report.get("config") or {}).get("BENCHMARK_LEVELS")],
        ["benchmark.levels_completed", len(benchmark.get("per_level") or [])],
        ["request_gap.matched_request_count", gap.get("matched_request_count")],
        ["request_gap.unmatched_request_count", gap.get("unmatched_request_count")],
        ["request_gap.unmatched_percent", gap.get("unmatched_percent")],
        ["gthread_queue.sample_count", queue_sample_count],
        ["gthread_queue.expected_request_count", queue_expected_count],
        ["gthread_queue.missing_sample_count", queue_missing_count],
        ["outbox_drain.success", drain.get("success")],
        ["outbox_drain.counts", drain.get("counts")],
        ["cleanup_success", report.get("cleanup_success")],
    ]


def postgres_connection_telemetry_rows(report: dict[str, Any]) -> list[list[Any]]:
    telemetry = report.get("database_connection_telemetry") or {}
    rows: list[list[Any]] = []
    for snapshot in telemetry.get("snapshots") or []:
        wait_event_present = snapshot.get("connections_wait_event_present")
        if wait_event_present is None:
            # Old reports used connections_waiting for wait_event IS NOT NULL.
            wait_event_present = snapshot.get("connections_waiting")
        active_wait_event = snapshot.get("connections_active_wait_event")
        lock_waiting = snapshot.get("connections_lock_waiting")
        idle_client_wait = snapshot.get("connections_idle_client_wait")
        rows.append([
            snapshot.get("stage"),
            snapshot.get("available"),
            snapshot.get("max_connections"),
            snapshot.get("connections_total"),
            snapshot.get("connections_active"),
            snapshot.get("connections_idle"),
            wait_event_present,
            active_wait_event,
            lock_waiting,
            idle_client_wait,
            snapshot.get("database_numbackends"),
...
def api_timing_rows(report: dict[str, Any]) -> list[list[Any]]:
    rows: list[list[Any]] = []
    by_endpoint = (report.get("api_timings") or {}).get("by_endpoint") or {}
...
def write_accurate_readme(
    ...
    *,
    ...
    target = concurrency_target(report, preferred=None)
    target_gap = gap_for_level(report, target)
    lines.extend(["", "## 3. Maximum-Tested-Concurrency Summary", ""])
    summary_rows = [
        ["summary_scope", "maximum_tested_concurrency"],
        ["maximum_tested_concurrency", target],
    ...
    lines.append(f"Server boundary source for this run: `{server_boundary_source(report)}`.")
    lines.append("")
    ...
    if post_view_rows:
        ...
        lines.append("")
        if metric(target_gap, "wsgi_response_iter_ms", "avg") is not None:
            lines.append(
                "`wsgi_response_iter_ms` covers WSGI response iterable consumption while Gunicorn writes response chunks; it is inside the server boundary, not end-to-end network latency."
            )
        else:
            lines.append(
                "`response_send_ms` is an ASGI application-boundary response emission interval, not end-to-end network latency."
            )
    else:
    ...
    if decomposition:
        ...
        if metric(target_gap, "server_structural_total_ms", "avg") is not None:
            lines.extend(
                [
                    "",
                    "`SERVER STRUCTURAL TOTAL` is an overlapping request-local "
                    "WSGI timing observation used only to reconcile pre-view + view "
                    "+ post-view. It must not be added to `SERVER TOTAL` or to "
                    "client latency.",
                ]
            )
    else:
    ...
    transaction_rows = transaction_timing_rows(target_gap)
    if transaction_rows:
        lines.extend(["", "**Transaction Boundary Timing**", ""])
        lines.extend(
            markdown_table(
                ["region", "avg_ms", "p50_ms", "p95_ms", "p99_ms", "max_ms"],
                transaction_rows,
            )
        )
        lines.append("")
        lines.append(
            "Transaction body timing contains the direct-send function body; "
            "commit/rollback timing measures the outer transaction.atomic exit boundary."
        )
    ...
    lines.extend(["", "## 15. Benchmark Harness And Validation", ""])
    lines.extend(markdown_table(["Check", "Value"], benchmark_harness_rows(report)))
    if report.get("fatal_error"):
        lines.extend(["", "**Fatal Error**", "", "```text", str(report.get("fatal_error")), "```"] )
    if report.get("traceback"):
        traceback_lines = str(report.get("traceback")).splitlines()
        lines.extend(["", "**Traceback Tail**", "", "```text", *traceback_lines[-40:], "```"] )

    lines.extend(["", "## 16. PostgreSQL Connection Telemetry", ""])
    postgres_rows = postgres_connection_telemetry_rows(report)
    if postgres_rows:
        ...
        lines.extend(
            [
                "",
                "`wait_event_present` counts sessions with any PostgreSQL wait event and is not equivalent to blocked queries. "
                "`active_wait_event` counts active sessions with a wait event. "
                "`lock_waiting` counts sessions waiting on PostgreSQL locks. "
                "`idle_client_wait` counts idle sessions waiting for the client.",
            ]
        )
    else:
        lines.append("No PostgreSQL connection telemetry snapshots were present.")

    lines.extend(["", "## 17. Docker Resource Usage", ""])
    ...
    if docker_rows:
        lines.extend(
            markdown_table(
                [
                    "container", "samples",
                    "cpu_min_pct", "cpu_avg_pct", "cpu_max_pct", "cpu_p95_pct",
                    "mem_min_pct", "mem_avg_pct", "mem_max_pct", "mem_p95_pct",
                    "mem_usage_avg_mb", "mem_usage_max_mb",
                    "net_rx_delta_mb", "net_tx_delta_mb",
                    "block_read_delta_mb", "block_write_delta_mb",
                    "pids_avg", "pids_max",
                ],
                docker_rows,
            )
        )
        lines.extend(["", "Docker stats are sampled by the host-side benchmark launcher. Sampling is observational and can add small monitoring overhead; per-level rows use the matched time window or the nearest container sample when a level is shorter than the sampling interval."])
    else:
        lines.append("No host Docker usage samples were available for this run.")

    lines.extend(["", "## 18. Recorded API Timing Diagnostics", ""])
    api_rows = api_timing_rows(report)
    if api_rows:
        lines.extend(markdown_table(
            ["endpoint", "calls", "success", "failure", "status_counts", "avg_ms", "p50_ms", "p95_ms", "p99_ms", "max_ms", "sample_errors"],
            api_rows,
        ))
        lines.extend(["", "These API timings include setup and cleanup traffic as well as benchmark-related endpoint calls. They are diagnostics and are not added to measured direct-send client latency unless the endpoint call itself is the measured send request."])
    else:
        lines.append("No API timing diagnostics were recorded.")
...
def concurrency_target(
    report: dict[str, Any],
    preferred: int | None = 100,
) -> int | None:
    levels = []
