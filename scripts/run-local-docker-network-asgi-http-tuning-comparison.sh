...
metadata_dir="$resolved_report_dir/asgi_http_tuning_comparison_$stamp"
...
common_messenger=(MESSENGER_PROCESS_ROLE=http GUNICORN_TIMEOUT=60 GUNICORN_GRACEFUL_TIMEOUT=30 GUNICORN_KEEP_ALIVE=5 GUNICORN_MAX_REQUESTS=1000 GUNICORN_MAX_REQUESTS_JITTER=100)
configs=("A|ASGI HTTP baseline|5|8|4096" "B|ASGI HTTP more workers|6|8|4096" "C|ASGI HTTP high workers|8|8|4096" "D|ASGI HTTP more ASGI threads|5|16|4096" "E|ASGI HTTP bigger backlog|5|16|8192")
metadata_paths=()

for config in "${configs[@]}"; do
  IFS='|' read -r id label web asgi_threads backlog <<<"$config"
  ...
  test_run_id="myna-asgi-http-$(printf '%s' "$stamp" | tr -cd '[:alnum:]')-$(printf '%s' "$id" | tr '[:upper:]' '[:lower:]')"
  messenger_env=("WEB_CONCURRENCY=$web" "ASGI_THREADS=$asgi_threads" "GUNICORN_BACKLOG=$backlog" "${common_messenger[@]}")
  runner_env=("MYNA_REPORT_FILE_PREFIX=myna_asgi_http_tuning_$slug" "MYNA_TEST_RUN_ID=$test_run_id" "${common_runner[@]}")
...
manifest_path="$metadata_dir/asgi_http_tuning_comparison_runs.json"
...
json.dump({"created_at": datetime.datetime.now(datetime.timezone.utc).isoformat(), "report_type": "asgi_http_tuning_comparison", "runs": runs}, open(sys.argv[1], "w", encoding="utf-8"), indent=2)
PY
combined_report_path="$resolved_report_dir/asgi_http_tuning_comparison_$stamp.md"
python3 ./scripts/build-asgi-http-tuning-comparison-report.py --runs-manifest "$manifest_path" --output-md "$combined_report_path"
pass "ASGI HTTP tuning comparison complete"
