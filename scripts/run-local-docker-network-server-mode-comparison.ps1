...
$metadataDir = Join-Path $resolvedReportDir "server_mode_comparison_$stamp"
...
$configs = @(
    @{ Id = "A"; Label = "ASGI baseline"; Mode = "asgi"; Web = "5"; AsgiThreads = "8"; GunicornThreads = "" },
    @{ Id = "B"; Label = "ASGI increased threads"; Mode = "asgi"; Web = "5"; AsgiThreads = "16"; GunicornThreads = "" },
    @{ Id = "C"; Label = "ASGI increased workers"; Mode = "asgi"; Web = "8"; AsgiThreads = "16"; GunicornThreads = "" },
    @{ Id = "D"; Label = "WSGI gthread baseline"; Mode = "wsgi"; Web = "4"; AsgiThreads = ""; GunicornThreads = "8" },
    @{ Id = "E"; Label = "WSGI gthread balanced"; Mode = "wsgi"; Web = "5"; AsgiThreads = ""; GunicornThreads = "8" },
    @{ Id = "F"; Label = "WSGI gthread high threads"; Mode = "wsgi"; Web = "5"; AsgiThreads = ""; GunicornThreads = "12" }
)
...
foreach ($config in $configs) {
    ...
    $testRunId = "myna-smc-$($stamp -replace '[^0-9A-Za-z]', '')-$($config.Id.ToLowerInvariant())"
    $metadataPath = Join-Path $metadataDir "$slug.run.json"

    $messengerEnv = @(
        "MESSENGER_HTTP_SERVER_MODE=$($config.Mode)",
        "WEB_CONCURRENCY=$($config.Web)"
    )
    if (-not [string]::IsNullOrWhiteSpace($config.AsgiThreads)) {
        $messengerEnv += "ASGI_THREADS=$($config.AsgiThreads)"
    }
    if (-not [string]::IsNullOrWhiteSpace($config.GunicornThreads)) {
        $messengerEnv += "GUNICORN_THREADS=$($config.GunicornThreads)"
    }
    $messengerEnv += $commonMessengerEnv

    $runnerEnv = @(
        "MYNA_REPORT_FILE_PREFIX=myna_server_mode_$slug",
    ...
    Write-Host "mode=$($config.Mode) WEB_CONCURRENCY=$($config.Web) ASGI_THREADS=$($config.AsgiThreads) GUNICORN_THREADS=$($config.GunicornThreads)" -ForegroundColor Yellow
    Write-Host "============================================================" -ForegroundColor Cyan
...
$manifestPath = Join-Path $metadataDir "server_mode_comparison_runs.json"
$manifest = [ordered]@{
    created_at = (Get-Date).ToString("o")
    report_type = "server_mode_comparison"
    runs = $runMetadata
}
$manifest | ConvertTo-Json -Depth 30 | Set-Content -Encoding UTF8 $manifestPath

$combinedReportPath = Join-Path $resolvedReportDir "server_mode_comparison_$stamp.md"
python ".\scripts\build-server-mode-comparison-report.py" `
    --runs-manifest $manifestPath `
    --output-md $combinedReportPath

if ($LASTEXITCODE -ne 0) {
    throw "Server-mode comparison report generation failed."
}

Write-Host ""
Write-Host "Server-mode comparison complete." -ForegroundColor Green
