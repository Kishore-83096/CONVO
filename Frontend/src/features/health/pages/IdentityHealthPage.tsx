...
import { useIdentityHealthAll } from "../hooks";
import { PageShell } from "../../../shared/ui/PageShell";

export function IdentityHealthPage() {
  const healthQuery = useIdentityHealthAll();
  const result = healthQuery.data;

  return (
    <PageShell
      eyebrow="Sprint 0 Phase 0.8"
      title="Identity Health"
      description="This page checks the public Identity service health endpoint."
    >
      <div className="health-panel">
        <div className="health-row">
          <strong>Endpoint</strong>
          <span>GET /health/all</span>
        </div>
        ...
        {result ? (
          <div className={result.ok ? "health-success" : "health-error"}>
            ...
            />
            <div>
              <strong>{result.ok ? "Identity service is online" : "Identity service needs attention"}</strong>
        ...
        {!healthQuery.isLoading && result && !result.ok ? (
          <div className="notice">
            Identity backend may not be running, the URL may be wrong, or CORS
