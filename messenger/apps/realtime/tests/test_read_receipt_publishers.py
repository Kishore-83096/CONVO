...
from apps.realtime.events import MESSAGE_READ
...
class ReadReceiptPublisherTests(TransactionTestCase):
    reset_sequences = True

    sender_device_id = uuid.UUID(
        "11111111-1111-4111-8111-111111111111"
    )
    reader_device_id = uuid.UUID(
    ...
    def setUp(self):
        ...
        self.reader_device = Device.objects.create(
            id=self.reader_device_id,
            user_id="2",
            device_name="Reader browser",
            platform=Device.Platform.WEB,
            registration_id=20001,
            identity_key_public="READER_IDENTITY",
            signed_prekey_id=1,
            signed_prekey_public="READER_SIGNED_PREKEY",
            signed_prekey_signature="READER_SIGNATURE",
            key_algorithm="curve25519",
            key_bundle_version=1,
            is_active=True,
        )
        ...
        self.receipt = MessageReceipt.objects.create(
            ...
            recipient_user_id="2",
            recipient_device=self.reader_device,
            delivered_at=timezone.now(),
            read_at=timezone.now(),
        )

    async def test_publish_message_read_sends_to_sender_device_group(self):
        ...
        self.assertEqual(payload["type"], MESSAGE_READ)
        ...
        self.assertEqual(data["reader_user_id"], "2")
        self.assertEqual(
            data["reader_device_id"],
            str(self.reader_device_id),
        )
    ...
    async def test_publish_message_read_is_skipped_when_reader_policy_suppresses_sender(self):
        """
        User 2 ghosts user 1.

        Even if a read receipt exists, publisher must not send message.read
        to user 1.
        """
